...
\begin{figure}[h]
	\includegraphics[width=1.0\linewidth]{../../Documents/Tirocinio/ImgTesi/provahashratelog}
	\caption{}
...
\begin{figure}
	\centering
	\includegraphics[width=1.0\linewidth]{../../Documents/Tirocinio/ImgTesi/LitecoinHashRateLIN}
	\caption{}
...
In conclusione: circa il 60\% dell’hashing power del 2017 è risultato concentrato nelle mani dei pools noti con la seguente suddivisione
...
Per ogni singolo anno (i dati prima del 2014 non sono significativi) si possono vedere affiancati in proporzione i blocchi frutto di merged mining (grigio) e non (arancione).
...
Questo grafico indica, per ogni miner, qual è la percentuale di blocchi vuoti rispetto ai blocchi totali

Nonostante il bug citato nel cap 4.1 relativo ai mining pools che impedisce la corretta attribuzione di alcuni blocchi ai rispettivi miners, la percentuale di blocchi vuoti minati da AntPool, BW.COM e F2Pool tra quelli rilevati è visibilmente degna di nota. L’osservazione tramite un explorer esterno (nella fattispecie litecoinpool.org in collaborazione con chain.so) ha permesso di riscontrare che AntPool tende a minare anche svariati blocchi vuoti consecutivi, dimostrandosi sotto questo punto di vista uno tra i pools più negativi.
In assoluto, la distribuzione dei blocchi vuoti tra i pools è la seguente:
...
A titolo d’esempio, la struttura di un OP\_RETURN di Omni Layer.
