...
\begin{tabular}{|c|c|c|c|}
	...
	\textbf{Bytes} &       \textbf{Name}        & \textbf{Data Type} &                     \textbf{Description}                     \\ \hline
	      4        &          version           &      int32\_t      &   Indicates which set of block validation rules to follow.   \\ \hline
	      32       & previous block header hash &      char[32]      &            A hash of the previous block’s header.            \\ \hline
	      32       &      merkle root hash      &      char[32]      &                A hash in internal byte order.                \\ \hline
	      4        &            time            &     uint32\_t      & A Unix epoch time when the miner started hashing the header. \\ \hline
	      4        &           nBits            &     uint32\_t      &  \\ \hline
	      4        &           nonce            &     uint32\_t      &  \\ \hline
\end{tabular}}
\end{table}



% RIGA 1: Intestazione ok, nessuna modifica
% RIGA 2: version va bene così
% RIGA 3: This ensures no previous block can be changed without also changing this block’s header.
% RIGA 4: The merkle root is derived from the hashes of all transactions included in this block, ensuring that none of those transactions can be modified without modifying the header. 
% RIGA 5: An encoded version of the target threshold this block’s header hash must be less than or equal to. Must be strictly greater than the median time of the previous 11 blocks. Full nodes will not accept blocks with headers more than two hours in the future according to their clock.
% RIGA 6:  An arbitrary number miners change to modify the header hash in order to produce a hash less than or equal to the target threshold. If all 32-bit values are tested, the time can be updated or the coinbase transaction can be changed and the merkle root updated.
